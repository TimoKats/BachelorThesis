...
def export_csv(source, id, date_published, date_modified, sponsor, introduction, body):
...
class scraper:
    ...
    def get_sponsor(self):
        index_1 = self.text_2.find('"theme":')
        index_2 = self.text_2.find('",', index_1)
        sponsor = self.text_2[index_1+8:index_2+1]
        return sponsor
...
if __name__ == '__main__':
    export_csv('source', 'article_id', 'date_published', 'date_modified', 'sponsor', 'introduction', 'body')
        ...
        for id in file_id:
            if id is not "":
                ...
                if(len(id.get_date_published()) < 50) and not("articleBody" in id.get_body()):
                    export_csv('telegraaf', id.get_id(), id.get_date_published(), id.get_date_modified(), id.get_sponsor(), id.get_introduction(), id.get_body())
